    ...
    <div>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 md:py-24 overflow-hidden">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-16 items-center">
          ...
          <FadeIn direction="right" className="rounded-3xl shadow-2xl overflow-hidden bg-slate-100 h-[500px] w-full max-w-md relative group">
          ...
          <FadeIn direction="left" delay={0.2}>
            ...
            <a href="#" className="font-semibold text-sm uppercase tracking-wider text-primary-600 hover:text-primary-700 transition-colors flex items-center gap-2 group">
              Jetzt eine Anfrage stellen <span className="group-hover:translate-x-1 transition-transform">&rarr;</span>
            </a>
          </FadeIn>
      ...
      <section className="bg-slate-50 py-24 border-y border-slate-100">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
            ...
            {/* Phase 2 */}
            <FadeIn direction="up" delay={0.3} className="relative">
              <div className="flex items-center gap-4 mb-6">
                <div className="text-primary-600 font-medium text-sm flex items-center gap-2">
                  <span className="w-1.5 h-1.5 bg-primary-600 rounded-full"></span>
                  Phase 2
            ...
            {/* Phase 3 */}
            <FadeIn direction="up" delay={0.4} className="relative">
              <div className="flex items-center gap-4 mb-6">
                <div className="text-primary-600 font-medium text-sm flex items-center gap-2">
                  <span className="w-1.5 h-1.5 bg-primary-600 rounded-full"></span>
                  Phase 3
      ...
      {/* Costs Section */}
      <section className="py-24 max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center overflow-hidden">
        <FadeIn direction="up">
        ...
        <FadeIn direction="up" delay={0.2} className="flex flex-col gap-0 text-left">
          {costs.map((cost, index) => (
            <div key={index} className="border-b border-slate-200 overflow-hidden">
              <button 
                onClick={() => toggleAccordion(index)}
                className="w-full py-6 flex justify-between items-center text-lg font-bold text-[#1e293b] hover:text-primary-600 transition-colors group tracking-tight"
              ...
              <div 
                className={`transition-all duration-500 ease-[0.25,1,0.5,1] px-4 text-slate-600 leading-relaxed ${openIndex === index ? 'max-h-40 pb-6 opacity-100 translate-y-0' : 'max-h-0 opacity-0 -translate-y-2'}`}
              >
                {cost.content}
              </div>
            </div>
